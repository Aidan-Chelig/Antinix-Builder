...
let
  ...
  effectiveKernelVersion =
    if kernelVersion != null then
      kernelVersion
    else if effectiveKernel != null then
      (effectiveKernel.modDirVersion or effectiveKernel.version)
    else
      throw "mkInitrd: one of `nixosSystem` or `kernel` must be provided (or pass `kernelVersion` and `moduleTree` explicitly)";
  ...
  effectiveModules = if modules != null then modules else lib.unique (defaultModules ++ extraModules);

  effectiveOmitModules =
    if omitModules != null then omitModules else lib.unique (defaultOmitModules ++ extraOmitModules);

  effectiveFilesystems =
    if filesystems != null then filesystems else lib.unique (defaultFilesystems ++ extraFilesystems);

  effectiveDrivers = if drivers != null then drivers else lib.unique (defaultDrivers ++ extraDrivers);

  overlayInstallCommands = lib.concatMapStringsSep "\n" (
    cmd:
    let
      srcExpr = if cmd.name == "udevadm" then "\"$UDEVADM\"" else "\"${cmd.src}\"";
    in
    ''
  ...
  renderedExtraOverlayFiles = lib.concatMapStringsSep "\n" (
    file:
    let
  ...
  renderedExtraBinSymlinks = lib.concatMapStringsSep "\n" (
    name:
    let
  ...
  renderedExtraOverlayCommands = lib.concatStringsSep "\n" (
    extraOverlayCommandsList ++ lib.optional (extraOverlayCommands != "") extraOverlayCommands
  );
