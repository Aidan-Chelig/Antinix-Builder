...
let
  ...
  normalizeHome = name: user: user.home or (if name == "root" then "/root" else "/home/${name}");

  normalizeShell = _name: user: user.shell or "/bin/sh";

  normalizeDescription =
    name: user: if (user.description or "") != "" then user.description else name;

  normalizePrimaryGroup = name: user: user.group or (if name == "root" then "root" else name);
  ...
  inferGroupGids =
    ...
    let
      ...
      step =
        ...
        let
          group = groups.${name};
          gid = if group.gid != null then group.gid else state.nextGid;
          nextGid =
            if group.gid != null then builtins.max state.nextGid (group.gid + 1) else state.nextGid + 1;
        in
        {
          nextGid = nextGid;
          gids = state.gids // {
    ...
    let
      ...
      step =
        ...
        let
          ...
          nextUid =
            if user.uid != null then
              builtins.max state.nextUid (user.uid + 1)
            else if name == "root" then
              state.nextUid
            else
              state.nextUid + 1;
  ...
  mkGroupMembers = userName: user: user.extraGroups or [ ];
        ...
        map (
          ...
          in
          {
            ...
            value = {
              ...
              mode = if name == "root" then "0700" else "0755";
            };
          }
        ) (builtins.filter (name: users.${name}.createHome or false) userNames)
      );
    in
    {
